...
router.put("/:id", async (req: Request, res: Response) => {
  ...
  const { name, menuIds, addonIds } = req.body;
  ...
  const existingAddonsResult = await db.query(
    "select * from addons where addon_categories_id = $1 and is_archived = false",
    [Number(id)]
  );
  console.log(existingAddonsResult.rows);
  const existingAddonIds = existingAddonsResult.rows.map((item) => item.id);
  console.log("existingAddonIds", existingAddonIds);

  //removeAddons
  const removedAddonIds = existingAddonIds.filter(
    (item: number) => !addonIds.includes(item)
  );
  console.log("removedAddonIds", removedAddonIds);
  if (removedAddonIds.length) {
    removedAddonIds.forEach(async (item: number) => {
      await db.query(
        "update addons set is_archived = true where id = $1 and addon_categories_id = $2",
        [item, Number(id)]
      );
    });
  }
  ...
  //AddedAddons
  const addedAddons = addonIds.filter(
    (item: number) => !existingAddonIds.includes(item)
  );
  console.log("addedAddons", addedAddons);
  const addedAddonsWithPriceResult = await db.query(
    "select * from addons where id = ANY($1::int[])",
    [addedAddons]
  );
  const addedAddonsWithPrice = addedAddonsWithPriceResult.rows;
  console.log("addedAddonsWithPrice", addedAddonsWithPrice);
  if (addedAddonsWithPrice.length) {
    addedAddonsWithPrice.forEach(async (item: any) => {
      const is_archived_True = await db.query(
        "select * from addons where id = $1 and addon_categories_id = $2 and is_archived = true",
        [item.id, Number(id)]
      );
      const is_Archived_True = is_archived_True.rows;
      console.log("isArchivedTrue", is_Archived_True);
      if (is_Archived_True.length) {
        await db.query(
          "update addons set is_archived = false where id = $1 and addon_categories_id = $2",
          [item.id, Number(id)]
        );
      } else {
        await db.query(
          "insert into addons (name, price,addon_categories_id) values ($1, $2, $3)",
          [item.name, item.price, Number(id)]
        );
      }
    });
  }
